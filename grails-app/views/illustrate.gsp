<!DOCTYPE html>
<html>
<head>
	<meta name="layout" content="main" />
	<title>Illustrate</title>
</head>

<body>
	<h1>Illustrating <span class="orange">UN</span>PLEXING</h1>

	<h2>Links</h2>
	<p>
		Here are links to blog entries, stories, and comics that we feel
		really illustrate the point. Kind of helps to drive the point home.
	</p>

	<ul>
		<li><a href="http://xkcd.com/974/">xkcd - The General Problem</a></li>
		<li><a href="http://tatiyants.com/?p=1180">Introducing JS.js</a></li>
		<li><a href="http://steve-yegge.blogspot.com/2006/03/execution-in-kingdom-of-nouns.html">Execution in the Kingdom of Nouns</a></li>
		<li><a href="http://www.engineyard.com/blog/2011/the-number-one-trait-of-a-great-developer/">The Number One Trait of a Great Developer</a></li>
		<li><a href="http://yamlabs.com/blog/showing_not_telling.html">Showing, not telling (time wasting in meetings)</a></li>
		<li><a href="http://sealedabstract.com/rants/why-software-projects-are-terrible-and-how-to-not-fix-them/">Why Software Projects are Terrible and How Not To Fix Them</a></li>
		<li><a href="${resource(dir: 'media', file: 'scaledsimplicity.png')}">Scaled Simplicity</a></li>
	</ul>
</body>
</html>